...
1. Flutter's generated `flutter_bootstrap.js` registered `flutter_service_worker.js` through `serviceWorkerSettings`.
...
## Fix
- Added `web/flutter_bootstrap.js` that calls `_flutter.loader.load()` without Flutter `serviceWorkerSettings`.
...
- The GitHub Pages build now uses `--pwa-strategy=none` and verifies that the built Flutter bootstrap contains no `serviceWorkerSettings`.
- CI now verifies the v16 worker and single-worker bootstrap contract before deployment.
