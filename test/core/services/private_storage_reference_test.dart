...
void main() {
  group('PrivateStorageReference', () {
    test('encodes and parses a durable private storage reference', () {
      final value = PrivateStorageReference.encode(bucket: 'lawyer_documents', path: 'auth-id/document.jpg');
    ...
    test('does not interpret ordinary or malformed URLs as storage refs', () {
      expect(PrivateStorageReference.tryParse('https://example.com/file.jpg'), isNull);
      expect(PrivateStorageReference.tryParse('private-storage://bucket/../secret'), isNull);
      expect(PrivateStorageReference.tryParse('private-storage://bucket'), isNull);
    });
  });
}
