-- Require a WhatsApp contact on the client profile before creating a booking.
-- The booking stores the server-side profile value, not an arbitrary client-supplied value.
-- Remote bookings also require the lawyer to have a WhatsApp contact; office bookings do not.
...
begin
  ...
  select nullif(trim(whatsapp),'') into v_lawyer_whatsapp
  from public.lawyer_profiles
  where profile_id=p_lawyer_id and verified=true and availability=true;
  ...
  if v_is_custom then
    ...
    select elem into v_package from public.lawyer_profiles lp,
