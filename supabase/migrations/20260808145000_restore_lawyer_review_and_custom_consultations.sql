create or replace function public.create_booking(p_lawyer_id uuid, p_scheduled_at timestamptz, p_package_name text, p_consultation_type text, p_description text default null, p_document_url text default null, p_client_whatsapp text default null, p_slot_id uuid default null)
...
begin
  ...
  if not exists (select 1 from public.lawyer_profiles where profile_id=p_lawyer_id and verified=true and availability=true) then
    raise exception 'المحامي غير متاح للحجز حالياً';
  end if;
  ...
  values(v_user_id,p_lawyer_id,'بانتظار التأكيد',v_slot_starts_at,v_price,p_consultation_type,nullif(trim(p_description),''),p_document_url,nullif(trim(p_client_whatsapp),''),case when v_is_custom then 'استشارة مختلفة' else trim(p_package_name) end,v_description,v_duration,'لم تبدأ',false,null)
...
begin
  ...
  if v_booking.status not in ('بانتظار التأكيد','قيد انتظار الدفع','قيد معالجة الدفع') or v_booking.lawyer_approved then raise exception 'لا يمكن مراجعة هذا الطلب في حالته الحالية'; end if;
  update public.bookings set lawyer_approved=p_approved, lawyer_approved_at=case when p_approved then now() else null end, status=case when p_approved then 'قيد انتظار الدفع' else 'ملغي' end, cancelled_at=case when not p_approved then now() else null end where id=p_booking_id returning * into v_booking;
  return v_booking;
end;
$function$;
