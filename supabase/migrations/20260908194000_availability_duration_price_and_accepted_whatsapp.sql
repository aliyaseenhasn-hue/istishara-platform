alter table public.lawyer_availability_slots
  add column if not exists duration_minutes integer,
  add column if not exists price numeric(12,2);

update public.lawyer_availability_slots
set duration_minutes = greatest(15, least(180, coalesce(duration_minutes, round(extract(epoch from (ends_at - starts_at))/60)::integer, 30)))
...
alter table public.lawyer_availability_slots
  drop constraint if exists lawyer_availability_slots_duration_minutes_check;
alter table public.lawyer_availability_slots
  add constraint lawyer_availability_slots_duration_minutes_check
  check (duration_minutes between 15 and 180);

alter table public.lawyer_availability_slots
  drop constraint if exists lawyer_availability_slots_price_check;
alter table public.lawyer_availability_slots
  add constraint lawyer_availability_slots_price_check
...
begin
  ...
  if not v_free_beta and not v_payments_enabled then raise exception 'إنشاء الحجوزات متوقف مؤقتاً حتى تفعيل وسيلة الدفع'; end if;
  ...
  if p_slot_id is not null then
    ...
    into v_slot_id, v_slot_starts_at, v_slot_duration, v_slot_price
  ...
  v_initial_status := case when v_free_beta then 'قيد مراجعة المحامي' when v_mode = 'في المكتب' then 'بانتظار التأكيد' else 'قيد انتظار الدفع' end;
...
begin
  ...
  select id into v_client_id from public.profiles where auth_id = auth.uid() limit 1;
