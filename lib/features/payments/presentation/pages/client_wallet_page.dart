          ...
          children: [
            ...
            topups.when(
              ...
              data: (items) => items.isEmpty
                  ...
                  : Column(
                      children: items.map((item) => _TopupHistoryCard(item: item)).toList(),
                    ),
            ),
            const SizedBox(height: 22),
            ...
            ledger.when(
              ...
              data: (items) => items.isEmpty
                  ...
                  : Column(
                      children: items.map((item) => _LedgerCard(item: item)).toList(),
                    ),
            ),
          ],
        ),
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
        const Text('الرصيد المتاح', textAlign: TextAlign.right, style: TextStyle(color: Colors.white70)),
        const SizedBox(height: 6),
        Text('$available د.ع', textAlign: TextAlign.right, style: const TextStyle(color: Colors.white, fontSize: 28, fontWeight: FontWeight.w900)),
        const SizedBox(height: 14),
        Text('محجوز لطلبات المواعيد: $held د.ع', textAlign: TextAlign.right, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w700)),
      ]),
    );
  }
}
...
class _TopupCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final provider = settings['provider_name']?.toString().trim() ?? 'تحويل يدوي';
    return Card(
      elevation: 0,
      ...
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          const Text('شحن المحفظة', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w900)),
          const SizedBox(height: 6),
          Text('حوّل المبلغ أولاً، ثم ارفع الإيصال. اعتماد الإدارة يضيف الرصيد ولا يرتبط بأي موعد.', style: TextStyle(color: scheme.onSurfaceVariant, height: 1.5)),
          const SizedBox(height: 14),
...
class _InfoLine extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Padding(
        ...
        child: Row(children: [Text('$label: ', style: const TextStyle(fontWeight: FontWeight.w700)), Expanded(child: SelectableText(value, textAlign: TextAlign.end))]),
      );
}
  ...
  Widget build(BuildContext context) {
    ...
    return Card(
      child: ListTile(
        ...
        title: Text('${NumberFormat('#,##0', 'ar').format(amount)} د.ع', style: const TextStyle(fontWeight: FontWeight.w900)),
        subtitle: Text('${item['transaction_number'] ?? ''}${date == null ? '' : '\n${DateFormat('yyyy/MM/dd – hh:mm a', 'ar').format(date)}'}${item['admin_note'] == null ? '' : '\nملاحظة الإدارة: ${item['admin_note']}'}'),
...
class _NoticeCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
        ...
        child: Text(text, textAlign: TextAlign.right, style: const TextStyle(fontWeight: FontWeight.w700, height: 1.5)),
      );
}
...
class _ErrorCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Container(padding: const EdgeInsets.all(14), color: Theme.of(context).colorScheme.errorContainer, child: Text(text));
}
...
class _EmptyCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Card(child: Padding(padding: const EdgeInsets.all(22), child: Text(text, textAlign: TextAlign.center)));
}
